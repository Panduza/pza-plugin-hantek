...
impl BooleanAccessorModel for SpecializedInterface {
    ...
    async fn get_boolean_at(&mut self, index: usize) -> Result<bool, Error> {
        ...
        // Perform the request
        match idx {
            BooleanAccessorIndex::Channel1Display => Ok(helper::scpi::ScpiBoolean::from_bytes(
                self.base
                    .lock()
                    .await
            ...
            .value()),
            BooleanAccessorIndex::Channel2Display => Ok(helper::scpi::ScpiBoolean::from_bytes(
                self.base
                    .lock()
                    .await
                    .ask(bytes::Bytes::from("CHANnel2:DISPlay?"))
                    .await?,
            )?
            .value()),
    ...
    async fn set_boolean_at(&mut self, index: usize, new_value: bool) -> Result<(), Error> {
        ...
        // Perform the request
        match idx {
            BooleanAccessorIndex::Channel1Display => {
                self.base
                    .lock()
                    .await
            ...
            BooleanAccessorIndex::Channel2Display => {
                self.base
                    .lock()
                    .await
                    .tell(bytes::Bytes::from(format!(
...
impl NumberAccessorModel for SpecializedInterface {
    ...
    async fn get_number_at(&mut self, index: usize) -> Result<f32, Error> {
        ...
        // Perform the request
        match idx {
            NumberAccessorIndex::Channel1Offset => Ok(helper::scpi::ScpiNumber::from_bytes(
                self.base
                    .lock()
                    .await
            ...
            .value()),
            NumberAccessorIndex::Channel1Scale => Ok(helper::scpi::ScpiNumber::from_bytes(
                self.base
                    .lock()
                    .await
            ...
            .value()),
            NumberAccessorIndex::Channel1Probe => Ok(helper::scpi::ScpiNumber::from_bytes(
                self.base
                    .lock()
                    .await
            ...
            .value()),
            NumberAccessorIndex::SecPerDiv => Ok(helper::scpi::ScpiNumber::from_bytes(
                self.base
                    .lock()
                    .await
                    .ask(bytes::Bytes::from("TIMebase:SCALe?"))
                    .await?,
            )?
            .value()),
            NumberAccessorIndex::TimebaseOffset => Ok(helper::scpi::ScpiNumber::from_bytes(
                self.base
                    .lock()
                    .await
                    .ask(bytes::Bytes::from("TIMebase:POSition?"))
                    .await?,
            )?
            .value()),
    ...
    async fn set_number_at(&mut self, index: usize, new_value: f32) -> Result<(), Error> {
        ...
        // Perform the request
        match idx {
            NumberAccessorIndex::Channel1Offset => {
                self.base
                    .lock()
                    .await
                    .tell(bytes::Bytes::from(format!(
            ...
            NumberAccessorIndex::Channel1Scale => {
                self.base
                    .lock()
                    .await
                    .tell(bytes::Bytes::from(format!(
            ...
            NumberAccessorIndex::Channel1Probe => {
                self.base
                    .lock()
                    .await
                    .tell(bytes::Bytes::from(format!(
            ...
            NumberAccessorIndex::TimebaseOffset => {
                self.base
                    .lock()
                    .await
                    .tell(bytes::Bytes::from(format!(
...
impl TriggerAccessorModel for SpecializedInterface {
    ...
    async fn trigger_at(&mut self, index: usize) -> Result<(), Error> {
        ...
        // Perform the request
        match idx {
            TriggerAccessorIndex::TriggerForce => {
                self.base
                    .lock()
                    .await
                    .tell(bytes::Bytes::from("TRIGger:FORCe"))
                    .await
            }
